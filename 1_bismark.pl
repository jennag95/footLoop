...
sub sanity_check {
	...
	($err .= err() . " -i Gene index [BED4+] $CY$opt_i$N doesn't exist!\n" and $die = 1) if defined($opt_i) and not -e $opt_i;
	($err .= err() . " -r Pacbio Read [FastQ] $CY$opt_r$N doesn't exist!\n" and $die = 1) if defined($opt_r) and not -e $opt_r;
	($err .= err() . " -g Genome [FastA] $CY$opt_g$N doesn't exist!\n" and $die = 1) if defined($opt_g) and not -e $opt_g;
	($err .= err() . " -x Has to be integer\n" and $die = 1) if defined($opt_x) and $opt_x !~ /^\-?\d+$/;
	($err .= err() . " -y Has to be integer\n" and $die = 1) if defined($opt_y) and $opt_y !~ /^\-?\d+$/;
	my $x = 0 if not defined($opt_x);
	my $y = 0 if not defined($opt_y);
	if ($die == 1) {
		print OUTLOG $usage . "\nSANITY:\n$log\n\nERRORS:\n$err\n";
		die $usage . "\nSANITY:\n$log\n\nERRORS:\n$err\n";
	}
	...
	if (defined($opt_x) or defined($opt_y)) {
		($err, $log, $die) = MYSYS("bedtools_bed_change.pl -m -x $x -y $y -i $geneIndexes -o $geneIndexes\_$x\_$y\_bp.bed", $err, $log);
		$geneIndexes = "$opt_i\_$x\_$y\_bp.bed";
	}
	if ($die == 1) {
		print OUTLOG $usage . "\nSANITY:\n$log\n\nERRORS:\n$err\n";
		die $usage . "\nSANITY:\n$log\n\nERRORS:\n$err\n";
	...
	my $bismark_folder = "$ENV{HOME}/Bismark_indexes/footLoop/$md1";
	...
	my $mydir = "";
 	for (my $i = 0; $i < @folders; $i++) {
		...
		if (not -d $mydir) {
			$index = 0;
			($err, $log, $die) = MYSYS("mkdir $mydir", $err, $log);
			last if $die == 1;
		}
	}
	if ($die == 1) {
	...
	if ($index == 0 or not -e "$bismark_folder/$md1.txt") {
		($err, $log, $die) = MYSYS("/bin/cp $geneIndexes $bismark_folder/", $err, $log);
		...
		$log .= dat() . "\t${YW}1b. Running$CY bismark_genome_preparation$N\n";
		print STDERR "\t${YW}1b. Running$CY bismark_genome_preparation$N\n";
	  ($err, $log, $die) = MYSYS("bismark_genome_preparation --bowtie2 $bismark_folder && md5sum $geneIndexesFa > $bismark_folder/Bisulfite_Genome/md5sum.txt", $err, $log);
	  if ($die == 1) {
			print OUTLOG $usage . "\nSANITY:\n$log\n\nERRORS:\n$err\n";
			die $usage . "\nSANITY:\n$log\n\nERRORS:\n$err\n";
		}
		$log .= dat() . "\t${GN}SUCCESS$N: $CY$bismark_folder\/Bisulfite_Genome$N was created!\n";
		($err, $log, $die) = MYSYS("echo $md1 > $bismark_folder/$md1.txt", $err, $log);
	}
	else {
